...
const CONFIG_FILE    = "./bots/sadeusxmd/config.json";
...
async function startPairingSession(number, krinyxUserId = null) {
  ...
  if (!sock.authState.creds.registered) {
    await delay(1500);
    try {
      const raw   = await sock.requestPairingCode(number, 'SADEUS12');
      pairingCode = raw.match(/.{1,4}/g).join("-");
      console.log(`🔑 Code pairing SADEUS ${number}: ${pairingCode}`);
    } catch (err) {
  ...
  const autoClose = setTimeout(() => {
    if (!bots.get(number)?.connected) {
      ...
      bots.delete(number);
      pairingLocks.delete(number);
      console.log(`⏱️ Timeout pairing SADEUS ${number}`);
    }
  }, PAIRING_TIMEOUT);
  ...
  sock.ev.on("connection.update", async ({ connection, lastDisconnect }) => {
    if (connection === "open") {
      ...
      console.log(`✅ Bot SADEUS ${number} connecté`);
    }
    ...
    if (connection === "close") {
      ...
      if (status === DisconnectReason.loggedOut || retries >= MAX_RETRIES) {
        ...
        console.log(`❌ Bot SADEUS ${number} déconnecté définitivement`);
      } else {
        retryCount.set(number, retries + 1);
        console.log(`🔁 Reconnexion SADEUS ${number} (${retries + 1}/${MAX_RETRIES})`);
        setTimeout(async () => {
          try { sock.end(); } catch {}
          bots.delete(number);
          pairingLocks.delete(number);
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    ...
    const credsPath = `./bots/sadeusxmd/sessions/${botNumber}/creds.json`;
...
// GET /sadeusxmd/pair/code?number=237XXXXXXXX&userId=123
...
// POST /sadeusxmd/pair/config  { number, prefix }
