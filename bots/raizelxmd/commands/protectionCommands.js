// ✅ commands/protectionCommands.js — version RAIZEL uniforme
import fs from "fs";
...
export default (() => {
  ...
  const toggleProtection = (key, value) => {
    statusProtections[key] = !!value;        // ← Intégré avec statusProtections
    return statusProtections[key];
  };
  ...
  return protections.map((key) => ({
    ...
    execute: async (sock, ctx, args) => {
      try {
        ...
        const currentStatus = statusProtections[key] || false;
